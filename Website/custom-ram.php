...
<!-- Header, logo and navigation bar section -->
  ...
  <main>
    <h3>Component Selector - Memory / RAM</h3>
        <div class="col-lg-3">
          <h4>Filter Products</h4><Br>
          <hr>
          <h6>RAM Speed Filter</H6>

            <?php
            $memoryResult = $link->query("SELECT DISTINCT speed from memoryram");
             ?>

            <form method="POST">
             <select name="memoryspeed">
              <option value="" disabled selected>Choose option</option>
               <?php
               while($rows = $memoryResult->fetch_assoc()){
                 $ramspeed = $rows['speed'];
                 echo "<option value='$ramspeed' selected='selected'>$ramspeed</option>";
               }
                ?>
             </select><br><br>
            ...
            if(isset($_POST["add_to_config"]))
            {
              if(isset($_SESSION["currentConfiguration"]))
              {
                ...
                if(!in_array($_GET["id"], $item_array_id))
                {
                  ...
                  echo "<script> window.location='custom-final.php'</script>";
                }
                else {
               ...
               <?php
                if(!empty($_SESSION["currentConfiguration"]))
                {
                  ...
                  foreach($_SESSION["currentConfiguration"] as $keys => $values)
                  {
                    ...
                    echo "<td>";
                    echo "<a href='custom-ram.php?action=delete&id=".$values["item_id"] ."'>Remove</a>";
                    echo "</td>";
                    echo "</tr>";
                    echo "</tr>";
                  }
    ...
    <div class="col-lg-9">
      ...
      <div class="row" id="result">
        ...
        <?php
        if(!empty($_POST['memoryspeed']))
        {
          $selected = $_POST['memoryspeed'];
          $sql="SELECT * from memoryram WHERE speed = '$selected' ";
          $result = $link->query($sql);
          while($row=$result->fetch_assoc()){
            echo "<form method='post' action='custom-ram.php?action=add&id=".$row["ID"] ."' >";
            ...
            echo "</h5>";
            echo "<h6>";
            echo "Memory Capacity: " .$row['Capacity'] ."<br>";
            echo "Memory Speed: " .$row['Speed'] ."<br>";
      ...
      <button><a href="custom-final.php">Finish selection</a></button>
